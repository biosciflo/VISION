...
from PyQt5.QtWidgets import QWidget, QVBoxLayout,QTableWidgetItem, QComboBox
import sys
...
#GP Analyser
import Analyzer_V1_0_3 as analyzer
...
num_unique_colors = 15  # or any number you need
# Choose a palette that supports more unique colors
palette_name = "tab20"  # tab20 has 20 unique colors

# Generate the palette with the required number of unique colors
color_palette = sns.color_palette(palette_name, num_unique_colors)
...
color_map = {segment_number: color for segment_number, color in zip(segment_numbers, color_palette)}
...
from custom_combobox import CheckableComboBox
...
class WorkerThread(QThread):
    ...
    def run(self):
        try:
            ...
            specific_error_messages = [ "Found array with 0 sample(s) (shape=(0, 2)) while a minimum of 1 is required by DBSCAN.",
                                        "No objects to concatenate"]
            if str(e) in specific_error_messages:
                custom_error_message = "Error: It was not possible to detect single objects. Please check your masking and thresholding settings."
                print(custom_error_message)
                self.window.plainTextEdit_1.appendPlainText(time.asctime(time.localtime()))
                self.window.plainTextEdit_1.appendPlainText(custom_error_message)
...
def load_and_process_lif(fullpath): # not completley implemented and absolutely not tested
    metadata={}
    data={}
...
def load_and_process_obf(fullpath): # not completley implemented and absolutely not tested
    # Load an image file
    ...
    for i, stack in enumerate(obf.stacks[:]):
        ...
        metadata[i]['ChannelColors'] = ((xml["OME"]["Image"][i]["Pixels"]["Channel"]["@EmissionWavelength"])).tolist()
...
def load_and_process_nd2(fullpath):
    metadata = {
        "Pixelsize": None,
        "Nchannels": None,
        "ChannelColors": None,
    ...
    with ND2Reader(fullpath) as nd2:
        # Extract metadata
        metadata['Pixelsize'] = nd2.metadata.get('pixel_microns', None)*1e-6
        metadata['PixelsizeUnit'] = 'um'  # Assuming unit is micrometers
        metadata['Nchannels'] = nd2.sizes.get('c', 1)
    
        DimensionTime = nd2.sizes.get('t', 1)
        DimensionZ = nd2.sizes.get('z', 1)
        DimensionX = nd2.sizes['x']
        DimensionY = nd2.sizes['y']
    
        metadata['Dimensions'] = [DimensionTime, DimensionZ, DimensionX, DimensionY, metadata['Nchannels']]
    
        # Extract ChannelColors
        metadata['ChannelColors'] = nd2.metadata.get('channels', None)
        if metadata['ChannelColors'] is not None:
            # Ensure ChannelColors contains numerical values
            if all(isinstance(item, (int, float)) for item in metadata['ChannelColors']):
                metadata['ChannelColors'] = [str(int(item)) for item in metadata['ChannelColors']]
            else:
                metadata['ChannelColors'] = [str(item) for item in metadata['ChannelColors']]
    
        # Read the image data
        data = np.zeros((DimensionTime, DimensionZ, DimensionY, DimensionX, metadata['Nchannels']), dtype=nd2[0].dtype)
        
        for t in range(DimensionTime):
            for z in range(DimensionZ):
                for c in range(metadata['Nchannels']):
                    data[t, z, :, :, c] = nd2.get_frame_2D(c=c, t=t, z=z)
...
def openfile(fullfilepath):
    ...
    #     return data, metadata
        ...
        return data, metadata
    elif fullfilepath.endswith(('.nd2')):
        data,metadata=load_and_process_nd2(fullfilepath)
        return data, metadata
    else:
        print("Only .lsm, .czi, lif, nd2 & .ome.tif !!! Your Variation of this filefomate is currently not supported. Pls send us a test image and we will gladly incorporte it (.lif and .obf are prepeared but I had no test images so pls send me some and I can implement it)")
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        ui_file = join(getattr(sys, '_MEIPASS', dirname(sys.argv[0])), "ui/gui_V1.0.3.ui")  # Standalone
    ...
    # Convert sizes to dp - example for a label
        ...
        self.comboBox_18 = self.findChild(CheckableComboBox, "comboBox_18")
        self.comboBox_23 = self.findChild(CheckableComboBox, "comboBox_23")
        ...
        self.comboBox_9.currentIndexChanged.connect(self.filterEnable)
        self.comboBox_10.currentIndexChanged.connect(self.filterEnable_cyto)
        ...
        self.tableWidget.itemSelectionChanged.connect(self.Update_Membrane_Cytosol_Chnnel_Settings)
        ...
        self.comboBox.currentIndexChanged.connect(self.Save_Membranechannel_A)
        self.comboBox_2.currentIndexChanged.connect(self.Save_Membranechannel_B)
        self.comboBox_6.currentIndexChanged.connect(self.Save_Membranechannel_C)
        self.comboBox_7.currentIndexChanged.connect(self.Save_Membranechannel_D)
        self.comboBox_8.currentIndexChanged.connect(self.Save_ColocolisationChannel)
        
        self.comboBox_13.currentIndexChanged.connect(self.Save_Cytosolchannel_A)
        self.comboBox_14.currentIndexChanged.connect(self.Save_Cytosolchannel_B)
        self.comboBox_15.currentIndexChanged.connect(self.Save_Cytosolchannel_C)
        self.comboBox_16.currentIndexChanged.connect(self.Save_Cytosolchannel_D)
        
        self.pushButton_4.clicked.connect(self.Globalize_Channel_Settings)
        self.pushButton_5.clicked.connect(self.Globalize_Channel_Settings)
        ...
        self.Colocolisation_Channel_selected={}
...
#### for testing purposes
        ...
        global Membrane_Channels_selected_g
        global Colocolisation_Channel_selected_g
        global Cytosol_Channels_selected_g
    ...
    def test(self):
        selected_items1 = self.comboBox_18.checkedItems()
        print(selected_items1)
        global Results_g
        Results_g = self.Results
        global Membrane_Channels_selected_g
        Membrane_Channels_selected_g = self.Membrane_Channels_selected
        global Colocolisation_Channel_selected_g
        Colocolisation_Channel_selected_g = self.Colocolisation_Channel_selected
        global Cytosol_Channels_selected_g
        Cytosol_Channels_selected_g = self.Cytosol_Channels_selected
        global Membrane_Maskdata_g
    ...
    def initialize_membrane_channels(self, filename):
        if filename not in self.Membrane_Channels_selected:
            self.Membrane_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
    
    def initialize_coloc_channels(self, filename):
        if filename not in self.Colocolisation_Channel_selected:
            self.Colocolisation_Channel_selected[filename] = None
    
    def initialize_cytosol_channels(self, filename):
        if filename not in self.Cytosol_Channels_selected:
            self.Cytosol_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
    
    def Globalize_Channel_Settings(self):
        if self.tableWidget.currentRow() !=-1:
            self.block_combobox_signals(True)
        
            # Collect all items from the first column of the tableWidget
            row_count = self.tableWidget.rowCount()
            items = [self.tableWidget.item(row, 0).text() for row in range(row_count) if self.tableWidget.item(row, 0) != "NaN"]
        
            current_filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            # Apply globalization logic for Membrane_Channels_selected
            if current_filename in self.Membrane_Channels_selected:
                globalize = self.Membrane_Channels_selected[current_filename]
                filtered_entries = [entry for entry in globalize if entry != "NaN"]
        
                for filename in items:
                    if filename in self.Membrane_Channels_selected:
                        if all(entry in self.Metadata[filename]['ChannelColors'] for entry in filtered_entries):
                            self.Membrane_Channels_selected[filename] = globalize.copy()
                    else:
                        self.Membrane_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
                        if all(entry in self.Metadata[filename]['ChannelColors'] for entry in filtered_entries):
                            self.Membrane_Channels_selected[filename] = globalize.copy()
        
            # Apply globalization logic for Colocolisation_Channel_selected
            if current_filename in self.Colocolisation_Channel_selected:
                globalize = self.Colocolisation_Channel_selected[current_filename]
                for filename in items:
                    if filename in self.Colocolisation_Channel_selected:
                        if str(globalize) in self.Metadata[filename]['ChannelColors']:
                            self.Colocolisation_Channel_selected[filename] = globalize
                    else:
                        self.Colocolisation_Channel_selected[filename] = "NaN"
                        if str(globalize) in self.Metadata[filename]['ChannelColors']:
                            self.Colocolisation_Channel_selected[filename] = globalize
        
            # Apply globalization logic for Cytosol_Channels_selected
            if current_filename in self.Cytosol_Channels_selected:
                globalize = self.Cytosol_Channels_selected[current_filename]
                filtered_entries = [entry for entry in globalize if entry != "NaN"]
        
                for filename in items:
                    if filename in self.Cytosol_Channels_selected:
                        if all(entry in self.Metadata[filename]['ChannelColors'] for entry in filtered_entries):
                            self.Cytosol_Channels_selected[filename] = globalize.copy()
                    else:
                        self.Cytosol_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
                        if all(entry in self.Metadata[filename]['ChannelColors'] for entry in filtered_entries):
                            self.Cytosol_Channels_selected[filename] = globalize.copy()
    ...
    def Update_Membrane_Cytosol_Chnnel_Settings(self):
        if self.tableWidget.currentRow() !=-1:
            self.block_combobox_signals(True)
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            
            # Ensure the  channels are initialized
            self.initialize_membrane_channels(filename)
            self.initialize_coloc_channels(filename)
            self.initialize_cytosol_channels(filename)
        
            # Update comboBox selections based on the current file's settings
            if filename in self.Membrane_Channels_selected:
                self.update_combobox_selection(self.Membrane_Channels_selected[filename], [
                    self.comboBox,
                    self.comboBox_2,
                    self.comboBox_6,
                    self.comboBox_7,
            ...
            if filename in self.Colocolisation_Channel_selected:
                self.update_single_combobox_selection(self.Colocolisation_Channel_selected[filename], self.comboBox_8)
        
            if filename in self.Cytosol_Channels_selected:
                self.update_combobox_selection(self.Cytosol_Channels_selected[filename], [
                    self.comboBox_13,
                    self.comboBox_14,
                    self.comboBox_15,
                    self.comboBox_16,
    ...
    def Save_Membranechannel_A(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            if filename not in self.Membrane_Channels_selected:
                self.Membrane_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
        
            self.Membrane_Channels_selected[filename][0] = self.comboBox.currentText()

    
    def Save_Membranechannel_B(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            if filename not in self.Membrane_Channels_selected:
                self.Membrane_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
        
            self.Membrane_Channels_selected[filename][1] = self.comboBox_2.currentText()

    
    def Save_Membranechannel_C(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            if filename not in self.Membrane_Channels_selected:
                self.Membrane_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
        
            self.Membrane_Channels_selected[filename][2] = self.comboBox_6.currentText()
    
    def Save_Membranechannel_D(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            if filename not in self.Membrane_Channels_selected:
                self.Membrane_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
        
            self.Membrane_Channels_selected[filename][3] = self.comboBox_7.currentText()
    
    def Save_ColocolisationChannel(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            self.Colocolisation_Channel_selected[filename] = self.comboBox_8.currentText()
    
    def Save_Cytosolchannel_A(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            if filename not in self.Cytosol_Channels_selected:
                self.Cytosol_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
    
            self.Cytosol_Channels_selected[filename][0] = self.comboBox_13.currentText()
    
    def Save_Cytosolchannel_B(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            if filename not in self.Cytosol_Channels_selected:
                self.Cytosol_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
        
            self.Cytosol_Channels_selected[filename][1] = self.comboBox_14.currentText()
    
    def Save_Cytosolchannel_C(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            if filename not in self.Cytosol_Channels_selected:
                self.Cytosol_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
        
            self.Cytosol_Channels_selected[filename][2] = self.comboBox_15.currentText()
    
    def Save_Cytosolchannel_D(self, index):
        if self.tableWidget.currentRow() !=-1:
            filename = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            if filename not in self.Cytosol_Channels_selected:
                self.Cytosol_Channels_selected[filename] = ["NaN", "NaN", "NaN", "NaN"]
        
            self.Cytosol_Channels_selected[filename][3] = self.comboBox_16.currentText()
    
    def block_combobox_signals(self, block):
        self.comboBox.blockSignals(block)
        self.comboBox_2.blockSignals(block)
        self.comboBox_6.blockSignals(block)
        self.comboBox_7.blockSignals(block)
        self.comboBox_8.blockSignals(block)
        self.comboBox_13.blockSignals(block)
        self.comboBox_14.blockSignals(block)
        self.comboBox_15.blockSignals(block)
        self.comboBox_16.blockSignals(block)
        self.comboBox_18.blockSignals(block)
        self.comboBox_23.blockSignals(block)
    
    def update_combobox_selection(self, selections, comboboxes):
        for selection, combobox in zip(selections, comboboxes):
            if selection != "NaN":
                index = combobox.findText(selection)
                if index != -1:
                    combobox.setCurrentIndex(index)
    
    def update_single_combobox_selection(self, selection, combobox):
        if selection != "NaN":
            index = combobox.findText(selection)
            if index != -1:
                combobox.setCurrentIndex(index)
    ...
    def PlotRaw_initial(self):
        if self.tableWidget.currentRow() !=-1:
            ...
            self.update_comboboxes(self.Metadata[filename]['ChannelColors'])
            if self.tableWidget.item(self.tableWidget.currentRow(),0) != [] and filename in self.RawImages:
                #self.RawImages[filename],metadata=openfile(join(path, filename))
                dims=self.Metadata[filename]['Dimensions']
        ...
        try:
            ...
            if len(value_type) > 1:
                ...
                if value.strip().lower() == "nan":
                    ...
                    return "NaN"
            if len(value_type) > 2:
                if value_type[2] == list:
                    # Split the value by commas and strip any whitespace
                    values = [v.strip() for v in value.split(',')]
                    # Convert each value in the list to the specified type (assuming the secondary type is int or str)
                    converted_values = [value_type[1](v) for v in values]
    ...
    def Detailed_Membrane_Masking_onItemChanged(self, item):
        ...
        translation_table = {
                                ...
                                'Filter Type': {'key': 'filter_type', 'type': (str,)},  
                                'Filter Value': {'key': 'filter_val', 'type': (float,)},
                                ...
                                'Specific Channel Wavelength': {'key': 'lambdaThr', 'type': (int, str,list)},  # can be str or int
        ...
        if translation:
            ...
            if key == "Ttype":
                ...
                invalid_values = ["No Filter", "Gaussian", "Mode", "Median", "Mean", "Geometric_mean","Majority","Minimum","Maximum",
                                  "Sum","Gradient","Entropy"]
                if not newValue.isdigit() and newValue not in invalid_values:
                    self.plainTextEdit_1.appendPlainText(time.asctime(time.localtime()))
                    self.plainTextEdit_1.appendPlainText(f'The value "{newValue}" is not valid for "Filter Type". ( pls choose one of these: "No Filter", "Gaussian", "Mode", "Median", "Mean", "Geometric_mean","Majority","Minimum","Maximum","Sum","Gradient","Entropy")')
                    Error=False
            elif key == "filter_val":
                if not newValue.replace(".", "", 1).isdigit():
                    self.plainTextEdit_1.appendPlainText(time.asctime(time.localtime()))
                    self.plainTextEdit_1.appendPlainText(f"The value '{newValue}' is not valid for 'Filter Value'. (a float)")
                    Error=False
            elif key == "S_N":
                if not newValue.replace(".", "", 1).isdigit():
                    self.plainTextEdit_1.appendPlainText(time.asctime(time.localtime()))
                ...
                if newValue != []:
                    new_values_list = [value.strip() for value in newValue.split(',')]
                    for value in new_values_list:
                        if value not in combo_items:
                            self.plainTextEdit_1.appendPlainText(time.asctime(time.localtime()))
                            self.plainTextEdit_1.appendPlainText(f"The value '{value}' is not valid for 'Specific Channel Wavelength'. (NaN or a list of available channels e.g. [488,647])")
                            Error=False
                            ...
                            for T_index in range(self.tableWidget_3.columnCount()):
                                for key, value in self.Membrane_Maskdata[filename][self.comboBox_17.currentIndex()].items():
                                    ...
                                    if header_index is not None:
                                        ...
                                        item_value=item_value.translate(str.maketrans("", "", "['']"))
    ...
    def update_toglobal_values_membrane(self,filename=None, path=None):
        ...
        Standard_Values={"Ttype":self.comboBox_3.currentText(),
           ...
           "pxlSize":self.Metadata[filename]['Pixelsize'],
           ...
           "PixelDepth":self.Metadata[filename]['BitDepth'],
           "filter_type":"No Filter" if self.comboBox_9.currentText() == "No Filter" else (self.comboBox_9.currentText()),
           "filter_val": self.doubleSpinBox_12.value(),
           ...
           "lambdaThr":[] if not self.checkBox_27.isChecked() else self.comboBox_18.checkedItems(),
           }
        reverse_translation_table = {
        'Ttype': 'Thresholding Mode',
        'ValManual': 'Manual Cutoff Level',
        ...
        if self.Metadata[filename]:
            if "Dimensions" in self.Metadata[filename]:
                if self.Metadata[filename]['Dimensions'][0]>1: #its a T stack
                    self.tableWidget_3.setColumnCount(self.Metadata[filename]['Dimensions'][0])
                else: # Z or 3dim
                ...
                if self.Metadata[filename]['Dimensions'][1]>1:
                    self.comboBox_17.clear()
                    self.comboBox_17.addItems([str(x+1) for x in range(self.Metadata[filename]['Dimensions'][1])])
                else:
                    self.comboBox_17.clear()
                    
            elif "0" in self.Metadata[filename]: # for lif and obl
                for n in range(len(self.Metadata[filename])):
                    print(self.Metadata[filename][n]['Dimensions'])
            ...
            if self.tableWidget.currentItem() is not None:
                if self.tableWidget.currentItem().isSelected():
                    ...
                    if filename in self.Membrane_Maskdata:
                        ...
                        #image,metadata=openfile(join(path,filename))
                        if "Dimensions" in self.Metadata[filename]:
                            if self.Metadata[filename]['Dimensions'][0]>1 and self.Metadata[filename]['Dimensions'][1]>1 : #its a TZ stack
                                self.tableWidget_3.setColumnCount(self.Metadata[filename]['Dimensions'][0])
                                self.comboBox_17.addItems([str(x+1) for x in range(self.Metadata[filename]['Dimensions'][1])])
                                for T_index in range(self.tableWidget_3.columnCount()):
                                    for key, value in self.Membrane_Maskdata[filename][T_index][self.comboBox_17.currentIndex()].items():
                                        ...
                                        if header_index is not None:
                                            ...
                                            item_value=item_value.translate(str.maketrans("", "", "['']"))
                                            self.tableWidget_3.setItem(header_index,T_index, QTableWidgetItem(item_value))
                            elif self.Metadata[filename]['Dimensions'][0]>1 and not self.Metadata[filename]['Dimensions'][1]>1 : #its a T stack
                                self.tableWidget_3.setColumnCount(self.Metadata[filename]['Dimensions'][0])
                                for T_index in range(self.tableWidget_3.columnCount()):
                                    for key, value in self.Membrane_Maskdata[filename][T_index].items():
                                        ...
                                        if header_index is not None:
                                            ...
                                            item_value=item_value.translate(str.maketrans("", "", "['']"))
                                            self.tableWidget_3.setItem(header_index,T_index, QTableWidgetItem(item_value))
                            elif not self.Metadata[filename]['Dimensions'][0]>1 and self.Metadata[filename]['Dimensions'][1]>1 : #its a Z stack
                                self.tableWidget_3.setColumnCount(self.Metadata[filename]['Dimensions'][0])
                                self.comboBox_17.addItems([str(x+1) for x in range(self.Metadata[filename]['Dimensions'][1])])
                                for T_index in range(self.tableWidget_3.columnCount()):
                                    for key, value in self.Membrane_Maskdata[filename][self.comboBox_17.currentIndex()].items():
                                        ...
                                        if header_index is not None:
                                            ...
                                            item_value=item_value.translate(str.maketrans("", "", "['']"))
                                            self.tableWidget_3.setItem(header_index,T_index, QTableWidgetItem(item_value))
                            elif not self.Metadata[filename]['Dimensions'][0]>1 and not self.Metadata[filename]['Dimensions'][1]>1 : #its a Z stack
                                self.tableWidget_3.setColumnCount(self.Metadata[filename]['Dimensions'][0])
                                for T_index in range(self.tableWidget_3.columnCount()):
    ...
    def update_toglobal_values_cytosol(self, filename=None,path=None):
       ...
       Standard_Values={"Ttype":self.comboBox_21.currentText(),
          ...
          "PixelDepth":self.Metadata[filename]['BitDepth'],
          "filter_type":"No Filter" if self.comboBox_10.currentText() == "No Filter" else (self.comboBox_10.currentText()),
          "filter_val": self.doubleSpinBox_11.value(),
          ...
          "lambdaThr":[] if not self.checkBox_36.isChecked() else self.comboBox_23.checkedItems(),
          }
       reverse_translation_table = {
       'Ttype': 'Thresholding Mode',
       'ValManual': 'Manual Cutoff Level',
       ...
       if self.Metadata[filename]:
           if "Dimensions" in self.Metadata[filename]:
               if self.Metadata[filename]['Dimensions'][0]>1: #its a T stack
                   self.tableWidget_6.setColumnCount(self.Metadata[filename]['Dimensions'][0])
               else: # Z or 3dim
               ...
               if self.Metadata[filename]['Dimensions'][1]>1:
                   ...
                   self.comboBox_24.addItems([str(x+1) for x in range(self.Metadata[filename]['Dimensions'][1])])
               else:
                   self.comboBox_24.clear()
            ...
            if self.tableWidget.currentItem() is not None:
                if self.tableWidget.currentItem().isSelected():
                    ...
                    if filename in self.Cyto_Maskdata:
                        ...
                        # image,self.Metadata[filename]=openfile(join(path,filename))
                        Standard_Values={"Ttype":self.comboBox_21.currentText(),
                           "ValManual":int(self.spinBox_15.value()),
                           "compress": bool(self.checkBox_32.isChecked()),
                           "Kvalue": float(self.doubleSpinBox_23.value()),
    ...
    def plot_RawImage(self, img, key, vmin=None, vmax=None):
        # Raw Image
        ...
        if key != []:
            ...
            if self.checkBox_33.checkState():
                if img_slice is not None:
                    if vmax is None:
                        vmax = np.percentile(img_slice, 99)  # Automatic calculation of vmax
                    if vmin is None:
                        vmin = np.percentile(img_slice, 1)  # Automatic calculation of vmin if needed
                        
                    isns.imgplot(img_slice, ax=ax, cmap="gray", cbar=False, origin='upper', vmin=vmin, vmax=vmax)
            else:
                isns.imgplot(img_slice, ax=ax, cmap="gray", cbar=False, origin='upper')
    ...
    def Update_Stats_oject_membrane(self,params,morphology,key):
        if key !=[] and params !=[] and morphology!=[]:
    ...
    def plot_Cyto_GPPhasor_Object(self,phase,ObjectNr,key): # GP Phasor
        ...
        ax.clear()
        if key !=[] and phase[int(ObjectNr)-1] != []:
            if key !=[] and isinstance(phase[int(ObjectNr)-1][0], np.ndarray):
                ph=phase[int(ObjectNr)-1][1][:]
                ph=np.where(ph < 0, ph + 2*np.pi, ph)
                df = pd.DataFrame({'amplitude': phase[int(ObjectNr)-1][0][:], 'phase': ph})
...
#         self.doubleSpinBox_3.setEnabled(self.checkBox_9.checkState())
    ...
    def filterEnable(self):
        if self.comboBox_9.currentText() != "No Filter":
            self.doubleSpinBox_12.setEnabled(True)
        else:
            self.doubleSpinBox_12.setEnabled(False)
        
    def filterEnable_cyto(self):
        if self.comboBox_10.currentText() != "No Filter":
    ...
    def SaveAllInJSON(self,filename,savepath):
        ui_settings = {
            ...
            "Membrane Settings":{
                ...
                "Specific Channel for Masking Value":[] if not self.checkBox_27.isChecked() else ((self.comboBox_18.currentText())),
                ...
                "filter_type":"No Filte" if self.comboBox_9.currentText() == "No Filter" else self.comboBox_9.currentText(),
                "filter_val ":self.doubleSpinBox_12.value(),
            ...
            "Cytosol Settings":{
                ...
                "Specific Channel for Masking Value":[] if not self.checkBox_36.isChecked() else ((self.comboBox_23.currentText())),
                ...
                "Filter Type":self.comboBox_10.currentText(),
                "Filter Value":self.doubleSpinBox_11.value(),
    ...
    def LoadFiles(self):
        ...
        self.path, _ = QFileDialog.getOpenFileNames(None, "Open Image File", "", "Images (*.lsm *.czi *.lif *.nd2 *.ome.tif)")
        ...
        for file_path in self.path:
            image, metadata = openfile(file_path)
            if isinstance(image, dict):
                maxlen += len(image)
            else:
                maxlen += 1
    
        self.tableWidget.setRowCount(maxlen if maxlen > len(self.path) else len(self.path))
    
        for pathn, file_path in enumerate(self.path):
            image, metadata = openfile(file_path)
            ...
            if isinstance(image, dict):
                for n in range(len(image)):
                    subimage_name = f"{base_name}_subimage_{n+1}"
                    self._set_table_item(row, subimage_name, dir_name)
                    self.RawImages[subimage_name] = image[n]
                    self.Metadata[subimage_name] = metadata[n]
                    self.initialize_membrane_channels(subimage_name)
                    self.initialize_coloc_channels(subimage_name)
                    self.initialize_cytosol_channels(subimage_name)
                    row += 1
            else:
                self._set_table_item(row, base_name, dir_name)
                self.RawImages[base_name] = image
                self.Metadata[base_name] = metadata
                self.initialize_membrane_channels(base_name)
                self.initialize_coloc_channels(base_name)
                self.initialize_cytosol_channels(base_name)
                row += 1
    
        self._update_comboboxes(metadata)
    
        self._enable_widgets()

    def _set_table_item(self, row, name, dir_name):
        self.tableWidget.setItem(row, 0, QtWidgets.QTableWidgetItem(name))
        self.tableWidget.setItem(row, 1, QtWidgets.QTableWidgetItem(dir_name))
    
    def _update_comboboxes(self, metadata):
        if isinstance(metadata, dict) and 'ChannelColors' in metadata:
            Lambdachannel = metadata['ChannelColors']
        else:
            Lambdachannel = metadata[0]['ChannelColors']
        
        self.update_comboboxes(Lambdachannel)
    
    def _enable_widgets(self):
        comboboxes = [
            self.comboBox, self.comboBox_2, self.comboBox_6, self.comboBox_7,
            self.comboBox_13, self.comboBox_14, self.comboBox_15, self.comboBox_16,
            self.comboBox_18, self.comboBox_23, self.comboBox_3, self.comboBox_10,
            self.comboBox_9, self.comboBox_21
        ]
        checkboxes = [
            self.checkBox, self.checkBox_2, self.checkBox_3, self.checkBox_4,
            self.checkBox_5, self.checkBox_6, self.checkBox_24, self.checkBox_27,
            self.checkBox_36, self.checkBox_16, self.checkBox_15, self.checkBox_32,
            self.checkBox_34, self.checkBox_35, self.checkBox_37, self.checkBox_31
        ]
        spinboxes = [
            self.doubleSpinBox_4, self.doubleSpinBox_5, self.doubleSpinBox_6,
            self.doubleSpinBox_8, self.doubleSpinBox_17, self.doubleSpinBox_18,
            self.doubleSpinBox_19, self.doubleSpinBox_21
        ]
        buttons = [
            self.B_Run, self.pushButton_3, self.B_LoadFiles, self.B_LoadFolder,
    ...
    def update_comboboxes(self,Lambdachannel):
            self.comboBox.blockSignals(True)
            self.comboBox_2.blockSignals(True)
            self.comboBox_6.blockSignals(True)
            self.comboBox_7.blockSignals(True)
            self.comboBox_8.blockSignals(True)
            self.comboBox_13.blockSignals(True)
            self.comboBox_14.blockSignals(True)
            self.comboBox_15.blockSignals(True)
            self.comboBox_16.blockSignals(True)
            self.comboBox_18.blockSignals(True)
            self.comboBox_23.blockSignals(True)
            ...
            self.comboBox_8.blockSignals(False)
            self.comboBox_13.blockSignals(False)
            self.comboBox_14.blockSignals(False)
            self.comboBox_15.blockSignals(False)
            self.comboBox_16.blockSignals(False)
            self.comboBox_18.blockSignals(False)
            self.comboBox_23.blockSignals(False)
    ...
    def LoadFolder(self):
        ...
        allowed_extensions = ['.lsm', '.czi', '.lif', '.nd2', '.ome.tif']
        ...
        for file_name in self.Files:
            image, metadata = openfile(join(self.path, file_name))
            if isinstance(image, dict):
                maxlen += len(image)
            else:
                maxlen += 1
    
        self.tableWidget.setRowCount(maxlen if maxlen > len(self.Files) else len(self.Files))
        ...
        for file_name in self.Files:
            ...
            if isinstance(image, dict):
                for n in range(len(image)):
                    subimage_name = f"{base_name}_subimage_{n+1}"
                    self._set_table_item(row, subimage_name, self.path)
                    self.RawImages[subimage_name] = image[n]
                    self.Metadata[subimage_name] = metadata[n]
                    self.initialize_membrane_channels(subimage_name)
                    self.initialize_coloc_channels(subimage_name)
                ...
                self._set_table_item(row, base_name, self.path)
                self.RawImages[base_name] = image
                self.Metadata[base_name] = metadata
                self.initialize_membrane_channels(base_name)
                self.initialize_coloc_channels(base_name)
                ...
                if self.tableWidget.currentItem().isSelected():
                    ...
                    for i in  reversed(range(self.tableWidget.rowCount())):
                        if (self.tableWidget.item(i,0)).isSelected():
                            self.ClearEntry(self.tableWidget.item(self.tableWidget.currentRow(),0).text())
                            counter=counter+1
    ...
    def ClearEntry(self,filename):
        self.block_combobox_signals(True)
        self.Results[filename]={}
        self.Membrane_Channels_selected[filename]={}
        self.Colocolisation_Channel_selected[filename]={}
        self.Cytosol_Channels_selected[filename]={}
        self.Membrane_Maskdata[filename]={} #[filename][t][z][key]
        self.Cyto_Maskdata[filename]={}
        self.dims[filename]={}
        self.RawImages[filename]={}
        ...
        del self.Membrane_Channels_selected[filename]
        del self.Colocolisation_Channel_selected[filename]
        del self.Cytosol_Channels_selected[filename]
        del self.Membrane_Maskdata[filename]
        del self.Cyto_Maskdata[filename]
        del self.dims[filename]
        del self.RawImages[filename]
        del self.GPImage[filename]
        del self.FullImage_Parameters[filename]
        del self.Cyto_GPImage[filename]
        del self.GPImage_per_object[filename]
        del self.GPPhasor[filename]
        del self.Cyto_GPPhasor[filename]
        del self.Masks[filename]
        del self.Cyto_Masks[filename]
        del self.Intensities[filename]
        del self.Cyto_Intensities[filename]
        del self.nobjects[filename]
        del self.Object_Parameters[filename]
        del self.Object_Morphology[filename]
        del self.Object_Coordinates[filename]
        del self.Mask_Object_Coordinates[filename]
        del self.Intensities_per_object[filename]
        del self.Channellamda[filename]
        del self.phasex_object[filename]
        del self.phasey_object[filename]
        del self.GPPhasor_polar_all[filename]
        del self.Cyto_GPPhasor_polar_all[filename]
        del self.GPPhasor_polar_obj[filename]
        del self.key[filename]
        del self.ConnectSliders[filename]
        del self.Profile[filename]
        del self.MembraneSegments[filename]
        del self.Cytoprof[filename]
        del self.Cyto_Image[filename]
        del self.FullImage_Parameters_cyto[filename]
        del self.Object_Parameters_cyto[filename]
        del self.Cyto_Intensities_per_object[filename]
        del self.Cyto_GPPhasor_polar_obj[filename]
        self.block_combobox_signals(False)
    ...
    def find_unique_valid_letters(self,equation_string, combo_A, combo_B, combo_C, combo_D):
        ...
        if invalid_char:
            return False, f"Error: Invalid character '{invalid_char}' found in the text. ('E', 'I', 'N', 'O', 'Q', 'S' are reserved for mathematical operators). Please use A,B,C or D."
        try:
        ...
        value=self.comboBox_18.checkedItems()
        ...
        if self.checkBox_14.isChecked() and self.comboBox_8.currentText() == []:
            invalid_combos.append("'Colocalization Channel'")

        if invalid_combos:
            return False,f"Error Membrane Settings: Letter(s) {', '.join(invalid_combos)} are in use in the Equation, but corresponding ComboBox value(s) are not selected. Please select a Channel for {', '.join(invalid_combos)}"
        ...
        if invalid_combos:
            return False,f"Error Membrane Settings: Letter(s) {', '.join(invalid_combos)} are not 'NaN' but corresponding letter(s) are not used in the equation. Please select 'NaN' for {', '.join(invalid_combos)}"
    ...
    def find_unique_valid_letters_cyto(self,equation_string, combo_A, combo_B, combo_C, combo_D):
        ...
        value=self.comboBox_23.checkedItems()
        if isinstance(value, str):
            values = [v.strip() for v in value.split(',')]
        elif isinstance(value, list):
            values = [str(v).strip() for v in value]
        ...
        if self.checkBox_36.isChecked():
            if len(values)==0:
                invalid_combos.append("'Specific Cytosolic Thresholding Channel'")

        if invalid_combos:
            return False,f"Error Cytosol Settings: Letter(s) {', '.join(invalid_combos)} are in use in the Equation, but corresponding ComboBox value(s) are not selected. Please select a Channel for {', '.join(invalid_combos)}"
        ...
        if invalid_combos:
            return False,f"Error Cytosol Settings: Letter(s) {', '.join(invalid_combos)} are not 'NaN' but corresponding letter(s) are not used in the equation. Please select 'NaN' for {', '.join(invalid_combos)}"
                    ...
                    if self.tableWidget.currentItem().isSelected():
                        for i in range(self.tableWidget.rowCount()):
                            if (self.tableWidget.item(i,0)).isSelected():
                                ...
                                Lambdachannel=self.Metadata[filename]['ChannelColors']
                                dims=self.Metadata[filename]['Dimensions']
                                Data_py=self.RawImages[filename]
                                # print("--------------------------------------")
                                # print("--------------------------------------")
                                # print("Pixelsize")
                                ...
                                Result=RunAnalyser(    mode=mode,
                                                    ...
                                                    image=Data_py,
                                                    dims=dims,
                                                    varlist=self.Membrane_Channels_selected[filename],
                                                    ...
                                                    varlist_cyto=self.Cytosol_Channels_selected[filename], 
                                                    ...
                                                    Colocalization= (self.Colocolisation_Channel_selected[filename]) if self.checkBox_14.isChecked() and self.comboBox_8.currentText().isdigit() else [],
                        ...
                        for i in range(self.tableWidget.rowCount()):
                            if (self.tableWidget.item(i,0)).isSelected():
                                ...
                                if self.checkBox_20.isChecked():
                                    self.SaveBetaColorcodedImageasTif(Result,filename,savepath)
    ...
    def SaveBetaColorcodedImageasTif(self,Result,filename,savepath):
        self.GPImage[filename]
        self.key[filename]
        
        if self.key[filename] == "3dim":
            new_filename = join(savepath, filename.rsplit('.', 1)[0] + '.tiff')
            lsm.imwrite(new_filename, self.GPImage[filename].astype(np.float32), imagej=True, metadata={'axes': 'YX'})
        elif self.key[filename] == "Tstack":
            new_filename = join(savepath, filename.rsplit('.', 1)[0] + '.tiff')
            t_array = np.array(self.GPImage[filename]).astype(np.float32)
            lsm.imwrite(new_filename, t_array, imagej=True, metadata={'axes': 'TYX'})
        elif self.key[filename] == "Zstack":
            new_filename = join(savepath, filename.rsplit('.', 1)[0] + '.tiff')
            z_array = np.array(self.GPImage[filename]).astype(np.float32)
            lsm.imwrite(new_filename, z_array, imagej=True, metadata={'axes': 'ZYX'})
        elif self.key[filename] == "TZstack":
            new_filename = join(savepath, filename.rsplit('.', 1)[0] + '.tiff')
            tz_array = np.array(self.GPImage[filename]).astype(np.float32)
            lsm.imwrite(new_filename, tz_array, imagej=True, metadata={'axes': 'TZYX'})
