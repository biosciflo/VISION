...
#GP Analyser
import Analyzer_V1_0_1 as analyzer
    ...
    with czifile.CziFile(fullpath) as czi:
        ...
        for subdict in czi_metadata["ImageDocument"]["Metadata"]["Information"]["Image"]["Dimensions"]["Channels"]["Channel"]:
            if subdict ["@Name"].isdigit():
                ...
                metadata['ChannelColors'].append(((subdict["EmissionWavelength"])))
            else:
...
#for spectral ana multi chanell
    ...
    with lsm.TiffFile(fullpath) as tif:
        ...
        if "ChannelColors" in lsm_metadata and all(item.isdigit() for item in lsm_metadata["ChannelColors"]["ColorNames"]):
            metadata['ChannelColors'] = np.array([((item)) for item in lsm_metadata["ChannelColors"]["ColorNames"]]).tolist()
        else:
            metadata['ChannelColors'] = ((np.mean(lsm_metadata["ChannelWavelength"] , axis=1))*1e9).astype(str).tolist()
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        ui_file = join(getattr(sys, '_MEIPASS', dirname(sys.argv[0])), "ui/gui_V1.0.1.ui")  # Standalone
