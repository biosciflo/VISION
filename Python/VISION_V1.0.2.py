...
#GP Analyser
import Analyzer_V1_0_2 as analyzer
...
def openfile(fullfilepath):
    ...
    #     return data, metadata
        ...
        print("Only .lsm, .czi, & .ome.tif !!! Your Variation of this filefomate is currently not supported. Pls send us a test image and we will gladly incorporte it (.lif and .obf are prepeared but I had no test images so pls send me some and I can implement it)")
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        ui_file = join(getattr(sys, '_MEIPASS', dirname(sys.argv[0])), "ui/gui_V1.0.2.ui")  # Standalone
...
#### for testing purposes
    ...
    def Globalize_Channel_Settings(self):
        print("Globalize_Channel_Settings")
        if self.tableWidget.currentRow() !=-1:
            self.block_combobox_signals(True)
        
            # Collect all items from the first column of the tableWidget
            row_count = self.tableWidget.rowCount()
            items = [self.tableWidget.item(row, 0).text() for row in range(row_count) if self.tableWidget.item(row, 0) is not None]
            ...
            if current_filename in self.Membrane_Channels_selected:
                ...
                filtered_entries = [entry for entry in globalize if entry is not None]
                ...
                for filename in items:
                    if filename in self.Membrane_Channels_selected:
                        ...
                        print("Test")
                        self.Membrane_Channels_selected[filename] = [None, None, None, None]
                        if all(entry in self.Metadata[filename]['ChannelColors'] for entry in filtered_entries):
                            self.Membrane_Channels_selected[filename] = globalize.copy()
            ...
            if current_filename in self.Cytosol_Channels_selected:
                ...
                for filename in items:
                    if filename in self.Cytosol_Channels_selected:
                        ...
                        self.Cytosol_Channels_selected[filename] = [None, None, None, None]
                        if all(entry in self.Metadata[filename]['ChannelColors'] for entry in filtered_entries):
                            self.Cytosol_Channels_selected[filename] = globalize.copy()
    ...
    def Update_Membrane_Cytosol_Chnnel_Settings(self):
        print("Update_Membrane_Cytosol_Chnnel_Settings")
        if self.tableWidget.currentRow() !=-1:
            self.block_combobox_signals(True)
    ...
    def plot_IntesitieDist(self,img,key): # GP Phasor
        ...
        if key !=[]:
            ...
            ax.bar(df['wavelength'], df['intensites'], color='blue')
...
#         self.doubleSpinBox_3.setEnabled(self.checkBox_9.checkState())
    ...
    def LoadFiles(self):
        ...
        self.path, _ = QFileDialog.getOpenFileNames(None, "Open Image File", "", "Images (*.lsm *.czi *.lif *.obf *.ome.tif)")
    ...
    def find_unique_valid_letters(self,equation_string, combo_A, combo_B, combo_C, combo_D):
        ...
        if invalid_combos:
            return False,f"Error Membrane Settings: Letter(s) {', '.join(invalid_combos)} are in use in the Equation, but corresponding ComboBox value(s) are not selected."
    ...
    def find_unique_valid_letters_cyto(self,equation_string, combo_A, combo_B, combo_C, combo_D):
        ...
        if invalid_combos:
            return False,f"Error Cytosol Settings: Letter(s) {', '.join(invalid_combos)} are in use in the Equation, but corresponding ComboBox value(s) are not selected."
                        ...
                        for i in range(self.tableWidget.rowCount()):
                            if (self.tableWidget.item(i,0)).isSelected():
                                ...
                                Result=RunAnalyser(    mode=mode,
                                                    ...
                                                    image=Data_py,
                                                    dims=dims,
                                                    varlist=[(self.comboBox.currentText()),
                                                                (self.comboBox_2.currentText()),
                                                                (self.comboBox_6.currentText()),
                                                                (self.comboBox_7.currentText())],
                                                    ...
                                                    varlist_cyto=[(self.comboBox_13.currentText()),
                                                                (self.comboBox_14.currentText()),
                                                                (self.comboBox_15.currentText()),
                                                                (self.comboBox_16.currentText())], 
                                                    ...
                                                    Colocalization= (self.comboBox_8.currentText()) if self.checkBox_14.isChecked() and self.comboBox_8.currentText().isdigit() else [],
